...
适用场景：程序中的逻辑错误。前提条件：程序由自己编写并使用 `-g` 参数编译以生成调试信息。
...
| `set args` | 设置 `main` 函数命令行参数（在 `start`、`run` 之前执行） |
...
Makefile 用于管理项目构建。文件命名为 `makefile` 或 `Makefile`，通过 `make` 命令执行。
...
匹配当前工作目录下的所有 `.c` 文件，将文件名组成列表赋值给变量 `src`（如 `src = add.c sub.c div1.c`）。

```makefile
obj = $(patsubst %.c, %.o, $(src))
```

将参数 3 中匹配参数 1 的部分替换为参数 2（如 `obj = add.o sub.o div1.o`）。
...
| `$@` | 在规则的命令中，表示规则中的目标 |
| `$^` | 在规则的命令中，表示所有依赖条件 |
| `$<` | 在规则的命令中，表示第一个依赖条件；在模式规则中可依次取出依赖条件列表 |
...
| `-f` | 指定文件执行 `make` 命令（如 `make -f xxxx.mk`） |

作业：编写一个 Makefile，可将其所在目录下的所有独立 `.c` 文件编译生成同名可执行文件。
